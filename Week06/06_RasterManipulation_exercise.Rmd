...
inMemory(elevation) # returns FALSE
...
# verify that the CRS match
st_crs(mounds) == st_crs(elevation)

# Create a boundnig box mounds_bb around the mounds with st_make_grid()
...
# crop the elevation layer by the mounds object
...
* Compute the area of the survey with `st_area()` and save this object as `areas`. What units are these? m2.
...
mounds$elev  <- _______::______(elevation, mounds)
...
>>>>>>> f842691219a44e7f3b437d973c4201cb5abd23fe
}

# Align the extent of the two rasters with crop()
elevation <- crop(aster, prominence)

# Do the overlay using the above define f function in the `fun` argument. 
elevation_prom_overlay <- overlay(elevation, prominence, fun = f)
