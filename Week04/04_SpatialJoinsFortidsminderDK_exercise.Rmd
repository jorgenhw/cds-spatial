...
monuments <- read_sf("data/fortidsminder/anlaeg_all_25832.shp")


# Check the periodisation and type of the monuments
class(monuments$datering)
table(monuments$datering)
...
plot(st_geometry(municipalities_25832, vikings))
plot(monuments, add = TRUE, pch = 16, col = "red")
```
...
the_crs <- st_crs(municipalities_25832, asText = TRUE)



the_crs <- crs(municipalities_25832, asText = T)
vikings_crs <- st_transform(viking, crs = the_crs)

viking_neigh <- st_join(vikings_crs, municipalities_25832)
...
nb <- data.frame(("Nobelparken"), 
                 longitude=(10.2033485),
                 latitude=(56.174346))
