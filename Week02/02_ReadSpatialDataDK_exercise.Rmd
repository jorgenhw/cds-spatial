...
*Each layer consts of 6 features and are of type MULTIPOLYGON.*
2. What is the CRS value in these objects?
*ETRS89 / UTM zone 32N*
CRS stands for Coordinate Reference System. It is a standard way of defining the location of spatial data on the earth's surface.
The value "ETRS89 / UTM zone 32N" is a CRS value that refers to the European Terrestrial Reference System 1989 and the *U*niversal *T*ransverse *M*ercator projection, zone 32N. This means that the spatial data is represented using a specific projection system that is optimized for the region around longitude 9°E and latitude 0°, which is known as UTM zone 32N.
...
length(parks_over_5h$areal_m2)
#17

# Limit to navn and areal_m2 variables
over5_lim <- parks_over_5h %>% select(navn, areal_m2) 
...
big_forests <- forests %>% filter(unclass(areas) > 20000)
...
```
In case a file does not have any crs data, you assign it manually like ```crs(play) <- 4326``` (4326 is just one of many)
...
* RRe-run the `plot()` code from the instruction above, color the `playgrounds` green and make the points stand out larger by setting the `lwd` argument to 3. You can also increase the transparency of `aarhus`, by setting its `alpha` at around 100.
...
plot(st_transform(playgrounds, crs = the_crs, col = "green"), add = TRUE, lwd = 3)
